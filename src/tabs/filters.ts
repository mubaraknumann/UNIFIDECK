...
export interface FilterParams {
  ...
  store: { store: "steam" | "epic" | "gog" | "amazon" | "ubisoft" | "microsoft" | "gamevault" | "all" };
...
export function updateUnifideckCache(
  ...
  console.log(
    `[Unifideck] Cache now has ${unifideckGameCache.size} entries (${games.length} games x2 for signed/unsigned)`,
  );
}
...
export function updateSingleGameStatus(game: {
  ...
  const unsignedId = signedId < 0 ? signedId + 0x100000000 : signedId;
  const altSignedId =
    signedId >= 0 && signedId > 0x7fffffff ? signedId - 0x100000000 : signedId;
...
export function updateValidThirdPartyCache(appIds: number[]) {
  console.log(
    `[Unifideck] Updating valid third-party shortcuts cache with ${appIds.length} shortcuts`,
  );
  ...
  console.log(
    `[Unifideck] Valid third-party cache now has ${validThirdPartyCache.size} entries`,
  );
}
...
export function getStoreForApp(
  appId: number,
  appType: number,
): "steam" | "epic" | "gog" | "amazon" | "ubisoft" | "microsoft" | "gamevault" | null {
