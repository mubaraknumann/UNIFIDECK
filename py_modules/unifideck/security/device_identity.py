...
class FakeDeviceIdentity:
    ...
    def __init__(self, value: str) -> None:
        """Create a fake reader returning ``value`` on every ``read``.

        Validates the input against
        ``DeviceIdentity._looks_valid`` so tests can't
        accidentally use an invalid id (which would
        only fail later during encryption). Lower-cases
        the value to match the real reader's
        normalisation.
        ...
        Raises:
            ValueError: if ``value`` doesn't look like
                a real machine-id.
        """
    ...
    def read(self) -> str:
        """Return the constructor-supplied value.

        Same signature as ``DeviceIdentity.read`` so
        the fake is drop-in substitutable. Always
        succeeds — no I/O, no caching needed (value
        is already pinned).

        Returns:
            The stored hex string.
        """
        return self._value
