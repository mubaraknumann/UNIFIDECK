...
| Issue | Resolution |
| --- | --- |
| Launch options keep resetting after a reboot. | As a temporary workaround, append your extra options to the end of the **Target** field instead. |
| LSFG does not activate. | Use `~/lsfg` or `LSFG=1`, and make sure the required LSFG pieces are actually installed and configured. The LSFG path was specifically fixed in 0.6.0. |
